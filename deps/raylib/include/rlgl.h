 ...
 *       #define RL_DEFAULT_BATCH_DRAWCALLS          256    // Default number of batch draw calls (by state changes:
 * mode, texture) #define RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS    4    // Maximum number of textures units that can be
 * activated on batch drawing (SetShaderValueTexture())
 *
 ...
 * 0 #define RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD     "vertexTexCoord"    // Bound by default to shader location: 1
 *       #define RL_DEFAULT_SHADER_ATTRIB_NAME_NORMAL       "vertexNormal"      // Bound by default to shader location:
 * 2 #define RL_DEFAULT_SHADER_ATTRIB_NAME_COLOR        "vertexColor"       // Bound by default to shader location: 3
 *       #define RL_DEFAULT_SHADER_ATTRIB_NAME_TANGENT      "vertexTangent"     // Bound by default to shader location:
 * 4 #define RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD2    "vertexTexCoord2"   // Bound by default to shader location: 5
 ...
 *         // texture0 (texture slot active 0) #define RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE1  "texture1"          //
 * texture1 (texture slot active 1) #define RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE2  "texture2"          // texture2
...
typedef struct rlDrawCall {
    ...
    // unsigned int vaoId;       // Vertex array id to be used on the draw -> Using
    // RLGL.currentBatch->vertexBuffer.vaoId unsigned int shaderId;    // Shader id to be used on the draw -> Using
    // RLGL.currentShaderId
...
RLAPI void rlBlitFramebuffer(int srcX, int srcY, int srcWidth, int srcHeight, int dstX, int dstY, int dstWidth,
                             int dstHeight, int bufferMask); // Blit active framebuffer to main framebuffer
...
RLAPI void
rlSetBlendFactorsSeparate(int glSrcRGB, int glDstRGB, int glSrcAlpha, int glDstAlpha, int glEqRGB,
...
RLAPI void
rlComputeShaderDispatch(unsigned int groupX, unsigned int groupY,
...
typedef struct rlglData {
    ...
    struct {
        ...
        int *currentShaderLocs;        // Current shader locations pointer to be used on rendering (by default,
                                       // defaultShaderLocs)
...
void rlVertex3f(float x, float y, float z) {
    ...
    // We must check current draw.mode when a new vertex is required and finish the batch only if the draw.mode
...
void rlglInit(int width, int height) {
    ...
    if ((glDebugMessageCallback != NULL) && (glDebugMessageControl != NULL)) {
        ...
        //  - GL_DEBUG_OUTPUT_SYNCHRONUS - Callback is in sync with errors, so a breakpoint can be placed on the
        //  callback in order to get a stacktrace for the GL error
...
void rlLoadExtensions(void *loader) {
    ...
    for (int i = 0; i < numExt; i++) {
        ...
        if (strcmp(extList[i], (const char *)"GL_OES_vertex_array_object") == 0) {
            ...
            // glIsVertexArray = (PFNGLISVERTEXARRAYOESPROC)loader("glIsVertexArrayOES");     // NOTE: Fails in WebGL,
            // omitted
    ...
    for (int eye = 0; eye < eyeCount; eye++) {
        ...
        if (RLGL.State.vertexCounter > 0) {
            ...
            float matMVPfloat[16]
                = { matMVP.m0, matMVP.m1, matMVP.m2,  matMVP.m3,  matMVP.m4,  matMVP.m5,  matMVP.m6,  matMVP.m7,
            ...
            // NOTE: Batch system accumulates calls by texture0 changes, additional textures are enabled for all the
            // draw calls
...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer) {
    ...
    // while other platforms using OpenGL ES 2.0 require/support sized internal formats depending on the GPU
    // capabilities
            ...
            if (data == NULL) {
                if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) {
                    if ((format == RL_PIXELFORMAT_UNCOMPRESSED_R32)
                        ...
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat, size, size, 0, glFormat,
                                     glType, NULL);
                } else
                ...
                if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
                    ...
                    glCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat, size, size, 0,
                                           dataSize, (unsigned char *)data + i * dataSize);
            }
...
void rlGenTextureMipmaps(unsigned int id, int width, int height, int format, int *mipmaps) {
    ...
    if ((texIsPOT) || (RLGL.ExtSupported.texNPOT)) {
        // glHint(GL_GENERATE_MIPMAP_HINT, GL_DONT_CARE);   // Hint for mipmaps generation algorithm: GL_FASTEST,
...
void *rlReadTexturePixels(unsigned int id, int width, int height, int format) {
    ...
    // boundary by default, which may add some padding. Use glPixelStorei to modify padding with the
    // GL_[UN]PACK_ALIGNMENT setting. GL_PACK_ALIGNMENT affects operations that read from OpenGL memory (glReadPixels,
    // glGetTexImage, etc.) GL_UNPACK_ALIGNMENT affects operations that write to OpenGL memory (glTexImage, etc.)
...
unsigned int rlLoadShaderCode(const char *vsCode, const char *fsCode) {
    ...
    // In case vertex and fragment shader are the default ones, no need to recompile, we can just assign the default
    // shader program id
    if ((vertexShaderId == RLGL.State.defaultVShaderId) && (fragmentShaderId == RLGL.State.defaultFShaderId))
        ...
        else
        {
            ...
            for (int i = 0; i < uniformCount; i++)
            {
                ...
                name[namelen] = 0;
                TRACELOGD("SHADER: [ID %i] Active uniform (%s) set at location: %i", id, name, glGetUniformLocation(id,
        name));
            }
        }
        */
...
int rlGetLocationUniform(unsigned int shaderId, const char *uniformName) {
    ...
    // if (location == -1) TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Failed to find shader uniform: %s", shaderId,
    // uniformName); else TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Shader uniform (%s) set at location: %i", shaderId,
...
int rlGetLocationAttrib(unsigned int shaderId, const char *attribName) {
    ...
    // if (location == -1) TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Failed to find shader attribute: %s", shaderId,
    // attribName); else TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Shader attribute (%s) set at location: %i", shaderId,
    // attribName, location);
#endif
    return location;
}
